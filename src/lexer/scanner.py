...
class Scanner:
    ...
    def skip_whitespace(self):
        """Skip spaces, tabs, and newlines (newline already handled in advance)."""
        while not self.is_at_end():
            ch = self.peek()
            if ch in ' \t\r':
                self.advance()
            elif ch == '\n':
                # newline already increments line/column in advance()
                self.advance()
            else:
                break
    ...
    def next_token(self) -> Token:
        ...
        # Return an error token so that parsing can continue (but we skip it)
